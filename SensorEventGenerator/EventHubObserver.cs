...
{
    class EventHubObserver : IObserver<FundCode>
    {
        ...
        public void OnNext(FundCode sensorData)
        {
            try
            {

                var serialisedString = JsonConvert.SerializeObject(sensorData);
                EventData data = new EventData(Encoding.UTF8.GetBytes(serialisedString)) { PartitionKey = sensorData.Fundcode };
                _eventHubClient.Send(data);
               
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine("Sending" + serialisedString + " at: " + sensorData.DateCreated);
